...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        q_values = map(q_value_func, actions)
        return max(q_values)
    ...
    def update(self, state, action, nextState, reward):
        ...
        update_value = self.alpha * (sample - current_q_value)
        updated_q_value = current_q_value + update_value
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        def scalar_multiply(counter, scalar):
            c = util.Counter()
            for key in counter:
                c[key] = counter[key] * scalar
            return c
        ...
        new_weights = current_weights + scalar_multiply(features, self.alpha * difference)
